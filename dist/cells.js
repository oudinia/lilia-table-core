 ...
 */
export function mathPretty(s) {
    let out = s;
    for (const k in MATH_MAP)
        out = out.split(k).join(MATH_MAP[k]);
 ...
 */
export function parseCellTokens(raw) {
    if (raw == null)
    ...
    while ((m = re.exec(raw))) {
        ...
        if (m[1] != null)
            out.push({ kind: 'bold', children: parseCellTokens(m[1]) });
        else
            out.push({ kind: 'math', value: m[2] });
        last = re.lastIndex;
    }
    if (last < raw.length)
...
export function cellToPlainText(raw) {
